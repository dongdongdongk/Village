...
class War {
  ...
  start() {
    ...
    console.log("│                                     │");
    console.log("│─────────────────────────────────────│");
    // 아스키 아트 출력을 비동기적으로 수행하고 완료되면 로그를 출력합니다.
    this.printAsciiArtFromFile("./ASCII/warVillage.txt")
      .then(() => {
        ...
        const attackerSoldiers = this.attacker.combatPower;
        const defenderSoldiers = this.defender.combatPower;
      ...
      .catch((error) => {
        console.error("아스키 아트 출력 중 오류가 발생했습니다:", error);
      });
  }
